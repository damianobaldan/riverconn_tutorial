...
After this tutorial is completed, the resulting  `igraph` object is meant to be used as input to functions to assess riverscape fragmentation. The [`riverconn`](https://github.com/damianobaldan/riverconn) R package allows implements a lot of such indices.  Most graph theory-based indices to asses fragmentation (see Jumani et al., 2020 for a review) conceptualize the riverscape as a graph where vertices (nodes) represent reaches, i.e. distintictly separated river sections; while edges (links) represent splitting objects, such as confluences (where two reaches join), barriers (items that disrupt the longitudial connectivity of the river), or habitat patches boundaries (borders between sections of the river with relatively uniform habitat conditions). This tutorial focuses on barriers (dams), but in principle other fragmentation inputs can be dealt with similarly.
...
This guide relies on several packages for data processing (the links point to useful resources and tutorials):
...
The river network shapefile can be retrieved from [HydroSHEDS](https://www.hydrosheds.org/page/overview), a global river network geo-referenced dataset derived from spaceborn elevation data. HydroSHEDS is widely used for global to regional analyses (Lehner et al., 2008). Data can be downloaded from [here](https://www.hydrosheds.org/downloads). River networks shapefiles (HydroRIVERS) are available at two different resolutions (15 and 30 sec). Basins and catchments shapefiles (HydroBASINS) are also available with 12 detail levels (Lehner et al., 2013). The Ebro catchment border can be selected from the polygons available in the HydroBASINS layer and the corresponding network can be extracted from the HydroSHEDS layer.
...
The obtained files can be imported in R (note that the river shapefile "Ebre_riv_modif.shp" has been sligtly modified from the original hydrosheds shapefile -see next sections on the checks carried out on the shapefiles. The original shapefile is saved in the Ebro_shape/ folder for comparison). 
```{r read shapes , message = FALSE, collapse = TRUE, width = 60, warning = FALSE}
shape_river<- st_read("Ebre_shape/Ebre_riv_modif.shp")
shape_basin <- st_read("Ebre_catchment/Ebro_catchment.shp")
shape_dams <- st_read("Ebre_Dams/Embalses/Embalses.shp")
```

## 1.4 Functions to support data processing

Some custom functions were developed and were used in the data processing. The source code and the documentation for these functions is found in Appendix B. Before starting, make sure those functions are loaded in the global environment: `get_outlet`, `create_network`, `snap_to_river`, `headwaters_dam`, `multiple_confluences`.
...
Shapefiles derived from the HydroRIVER layer have a high detail level: a network with a high detail level will have a large amount of nodes and links, making the algorithms for network analysis slow. Thus it makes sense to reduce the size of the network by pruning the most upstream reaches. The information on the upstream area (number of DEM raster cells) is stored in the field 'UP_AREA'. Knowing the HydroSHEDS resolution (data used here have a resoluton of 15 seconds, corresponding to 450 meters at this latitude), a threshold on upstream area can be decided.
...
shape_river_small <- shape_river[as.numeric(shape_river$UP_CELLS) > threshold,]
```

Next, the dams shapefile can be converted to a shapefile and a unique id can be given to each point. As Dams dataset is a "polygon" shapefile we calculate the centroid of each poligon to obtain a "point" layer. The reference system of the dams layer is changed to WGS84, the same reference system of HydroSHEDS.
...
The shapefies can be plot to check there are no errors in the geolocalization of the data.
...
ggplot() +
  ...
  ggspatial::layer_spatial(shape_river_small, aes(color = log10(as.numeric(UP_CELLS * 0.45 * 0.45))))+
...
Every river network contains confluences, i.e. points where two reaches merge into one. In the river graph defined above, confluences are edges (links) between nodes. To define confluences the pruned river shapefile is first simplified and its geometry is casted to "POINT".
...
The last step -this step is needed later- is the splitting of the simplified shapefile where it intersects a confluence. This allows to identify unequivocally the river sections that are comprised between two confluences. Such elements will then became vertices of the graph. The function `st_split` from the `lwgeom` package is used. This function splits an object of class 'sf' with 'MULTILINESTRING' geometry into and object of class 'sf' containing multiple 'LINESTRING' geometries records based on the position of the points. Note that for this function to work properly, the point object must perfectly overlap with the polyline (this can be checked with the function `sf::st_distance` applied to the points and the polyline, which must return a vector of zeroes).
...
The function `snap_to_river` (see Appendix B) is used to snap the dams points to the river network based on a tolerance threshold (here, 1000 m were used). After this function, the distance of snapped dams can be checked to make sure the algorithm worked properly.
...
The last step is to slice the river shapefile based on both the position of dams and confluences. First a combined 'junctions' dataset is created, then the function `lwgeom::st_split` is applied again. note that the confluences are assigned NA values to the fields present in the dams shapefile. Note also that in the river network shapefile, the field length is calculated.
...
river_net_simplified <- lwgeom::st_split(shape_river_simple, network_links) %>%
  st_collection_extract(.,"LINESTRING") %>%
  data.frame(EdgeID = 1:length(.), geometry = .) %>%
  st_as_sf() %>%
  mutate(length = st_length(.))
```

A plot can reveal if the process was succesful
...
Here some example of indices for riverscape connectivity applied to river graphs are presented.

## 4.1 Dams removal prioritization
The function `d_index_calculation` from the `riverconn` package is used to identify dams whose passability improvement can best benefit the catchment connectivity. Check the package vignette for details on the indices used to this end. Here we use different setups based on the habitat and the dispersal mode to check weather the prioritization changes.
...
lab_i[[1]] <- "Structural symmetric"
letter_i[[1]] <- "A"
d_i[[1]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                B_ij_flag = FALSE, 
                                parallel = FALSE)

# 2: asymmetric dendritic connectivity index (no biotic effects)
lab_i[[2]] <- "Structural asymmetric"
letter_i[[2]] <- "B"
d_i[[2]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata, 
                                B_ij_flag = FALSE, 
                                parallel = FALSE, 
                                dir_fragmentation_type = "asymmetric")

# 3: population connectivity index with lowland fish
lab_i[[3]] <- "Lowland fish"
letter_i[[3]] <- "C"
d_i[[3]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                weight = "WUL_low", 
                                param = 0.8, 
                                parallel = FALSE)

# 4: population connectivity index with highland fish
lab_i[[4]] <- "Highland fish"
letter_i[[4]] <- "D"
d_i[[4]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                weight = "WUL_high", 
                                param = 0.8, 
                                parallel = FALSE)

# 5: population connectivity index with lowland diadromous fish
lab_i[[5]] <- "Lowland diadromous"
letter_i[[5]] <- "E"
d_i[[5]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                index_type = "reach", 
                                index_mode = "to",
                                weight = "WUL_low", 
                                param = 0.8, 
                                parallel = FALSE) %>%
  dplyr::filter(name == as.character(outlet))

# 6: population connectivity index with highland diadromous fish
lab_i[[6]] <- "Highland diadromous"
letter_i[[6]] <- "F"
d_i[[6]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                index_type = "reach", 
                                index_mode = "to",
                                weight = "WUL_high", 
...
# 7: population connectivity index with lowland diadromous fish
## note that units of param_d must be consistent with length field (10s of km)
lab_i[[7]] <- "Lowland passive"
letter_i[[7]] <- "G"
d_i[[7]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                weight = "WUL_low", 
...
# 8: population connectivity index with highland passive drifter
## note that units of param_d must be consistent with length field (10s of km)
lab_i[[8]] <- "Highland passive"
letter_i[[8]] <- "H"
d_i[[8]] <- d_index_calculation(graph = river_graph, 
                                barriers_metadata = barriers_metadata,
                                weight = "WUL_high", 
                                dir_distance_type  = "asymmetric",
                                disp_type = "threshold",
...
for (i in 1:length(d_i)) {
  ...
  plot_list[[i]] <- ggplot() +
    ...
    ggspatial::layer_spatial(network_links_plot, 
                             aes(color = d_index, size = d_index), alpha = 0.8)+
    scale_color_viridis(name = "dI")+
    theme_void()+
    guides(size = FALSE) +
    ggtitle(letter_i[[i]])+
    labs(caption = lab_i[[i]])
}

plot_final <- gridExtra::grid.arrange(
  plot_list[[1]],
  plot_list[[2]],
  plot_list[[3]], 
  plot_list[[4]], 
...
d_index_chart <- data.frame("d_i_1" = d_i[[1]]$d_index, 
                            "d_i_2" = d_i[[2]]$d_index,
                            "d_i_3" = d_i[[3]]$d_index,
                            "d_i_4" = d_i[[4]]$d_index,
                            "d_i_5" = d_i[[5]]$d_index,
...
colnames(d_index_chart) <- c(letter_i[[1]], letter_i[[2]], letter_i[[3]], 
                             letter_i[[4]], letter_i[[5]], letter_i[[6]],
...
corrmorant::corrmorant(d_index_chart, corr_method = "spearman")
ggsave("Figures/ranking comparison.jpeg",  width = 19, height = 19, units = "cm")
```
