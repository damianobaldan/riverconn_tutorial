...
The funtion `index_calculation` from the [`riverconn`](https://github.com/damianobaldan/riverconn) package is used to calculate reach-scale connectivity indices (RCI). For comparison purposes, different indices are calculated: (A) Dendritic connectivity index with symmetric dams passabilities; (B) Dendritic connectivity index with asymmetric dams passabilities; (C) Integral Index of connectivity (dams are not passable and a threshold on dispersal distance is used); (D) Integral Index of connectivity with uniform weights; (E) lowland-preferring organisms with active aquatic dispersal (e.g. fish), (F) highland-preferring organisms with active aquatic dispersal (e.g. fish), (G) lowland-preferring organism with passive aquatic dispersal (e.g., invertebrate larval stage), (H) highland-preferring organism with passive aquatic dispersal (e.g., invertebrate larval stage, bivalve)
...
# 6: Population Connectivity Index for highland fish
lab_index[[6]] <- "PCI highland fish"
letter_index[[6]] <- "F"
...
# 8: Population Connectivity Index for highland passive drifter
## note that units of param_d must be consistent with length field (10s of km)
lab_index[[8]] <- "PCI highland passive"
letter_index[[8]] <- "H"
...
Resulting maps can be plotted to visually compare the results. For comparisons between different indices, reach ranks are plotted. The lowest rank is the total number of reaches (ca. 650).
...
ggsave("Figures/ranking_habitat_comparison.jpeg",  width = 19, height = 19, units = "cm")
```


## 4.2 Dams removal prioritization
The function `d_index_calculation` from the [`riverconn`](https://github.com/damianobaldan/riverconn) package is used to identify dams whose passability improvement can best benefit the catchment connectivity. Check the package vignette for details on the indices used to this end. Here we use the same setups described above (section 4.1).
...
# 6: Population Connectivity Index for highland fish
lab_d_index[[6]] <- "PCI highland fish"
letter_d_index[[6]] <- "F"
...
# 8: Population Connectivity Index for highland passive drifter
## note that units of param_d must be consistent with length field (10s of km)
lab_d_index[[8]] <- "PCI highland passive"
letter_d_index[[8]] <- "H"
...
Resulting maps can be plotted to visually compare the results. For comparisons between different indices, barriers ranks are plotted. The lowest rank is the total number of barriers (ca. 80).
...
colnames(d_index_chart) <- c(letter_d_index[[1]], letter_d_index[[2]], letter_d_index[[3]], 
                             ...
                             letter_d_index[[7]], letter_d_index[[8]])
