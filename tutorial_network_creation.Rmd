...
function). Another common problem in HydroSHEDS-derived shapefiles is the presence of gaps between segments. This issue can cause problems in the `network_creation` function, which expects a shapefile with no gaps. The function `gaps_detection` (Appendix B) can be used to detect such problematic points in the shapefile. 

Once edited, re-run the script and check again that there are no more four reaches confluences.   
